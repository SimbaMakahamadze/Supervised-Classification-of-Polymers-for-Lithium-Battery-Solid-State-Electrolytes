...
import logging
from helper import recursive_feature_elimination
...
fh = logging.FileHandler('logs\\forest.log', mode='a')
...
def forest():
    ...
    rf = RandomForestClassifier(max_depth=10, min_samples_leaf=3, 
                                 min_samples_split=8, bootstrap=True,
                                 max_features='auto', n_estimators=100
                                 )
    
    clf = recursive_feature_elimination(rf, X_train, y_train)
    t0 = time.time()
...
def main():
    ...
    dump(m, 'forest_rfecv_metrics.joblib')
