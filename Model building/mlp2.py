...
"""Trying to rank features for our ANN classifier"""
...
def mlp():
    ...
    rfecv = recursive_feature_elimination(clf, X_train, y_train)
    #rfecv.fit(X_train, y_train)
    t0 = time.time()
    y_pred = rfecv.predict(X_val, y_val)
    t1 = time.time()
    t = t1 - t0
    dump(rfecv, 'nn2.joblib')
